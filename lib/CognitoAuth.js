...
export default class CognitoAuth {
  ...
  getCodeQueryParameter(httpRequestResponse) {
    const map = this.getQueryParameters(httpRequestResponse, this.getCognitoConstants().QUESTIONMARK);
    const state = null;
  ...
  getTokenQueryParameter(httpRequestResponse) {
    const map = this.getQueryParameters(httpRequestResponse, this.getCognitoConstants().QUERYPARAMETERREGEX1);
    const idToken = new CognitoIdToken();
    const accessToken = new CognitoAccessToken();
    const refreshToken = new CognitoRefreshToken();
    const state = null;
